...
function emit.emit_bundle(context, bundle_info, output_directory_c, output_directory_hlsl)
	...
	io.write("// Generated by metagen.lua from " .. bundle_file_name .. ".metashader\n\n")
	...
	for _, flags_export in ipairs(flags) do
		local flags_name   = flags_export.name
		local flags_values = emit.table_to_sorted_array(flags_export.values)

		io.write("typedef uint32_t " .. flags_name .. ";\n")
		io.write("typedef enum enum_" .. flags_name .. "\n{\n")

		for _, v in pairs(flags_values) do
			local flag_name  = v.k
			local flag_value = v.v
			io.write("\t" .. flag_name .. " = " .. tostring(flag_value) .. ",\n")
		end
			
		io.write("} enum_" .. flags_name .. ";\n\n")
	end
	...
	local has_hlsli  = #cbuffers > 0 or #flags > 0
	...
	if has_hlsli then
		...
		for _, cbuffer in ipairs(cbuffers) do
	...
	local hlsl_source = "// Generated by metagen.lua from " .. bundle_file_name .. ".metashader\n\n"
