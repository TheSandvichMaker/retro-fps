...
typedef enum ui_event_kind_t
{
	...
	// events that can be generated by widgets
	UiEvent_focus_rectangle,
...
typedef enum ui_button_t
{
	...
	UiButton_x2,
	UiButton_any,
	UiButton_COUNT = UiButton_any,
} ui_button_t;

fn_local ui_buttons_t ui_button_to_mask(ui_button_t button)
{
	if (button == UiButton_any)
	{
		return 0xFF;
	}
...
typedef struct ui_input_t
{
	...
	union
	{
		...
		v2_t mouse_p_on_button[UiButton_COUNT];
	};
...
fn bool        ui_button_pressed (ui_button_t button, bool consume);
fn bool        ui_button_released(ui_button_t button, bool consume);
fn bool        ui_button_held    (ui_button_t button, bool consume);
...
typedef uint32_t ui_responder_flags_t;
...
fn void ui_push_responder_stack_ex(ui_id_t id, ui_responder_flags_t flags);
