...
void shader_set_params__ui_draw_parameters_t(rhi_command_list_t *list, uint32_t slot, ui_draw_parameters_t *params)
{
...
#define DF_SHADER_UI_SOURCE_CODE \
	...
	"}\n" \
	"\n" \
	"float4 ui_ps(VSOut IN) : SV_Target\n" \
	"{\n" \
	...
	"\n" \
	"	if (rect.flags & UiRectFlags_hue_picker)\n" \
	"	{\n" \
	"		float h = uv.y;\n" \
	"		color.rgb = rgb_from_hsv(float3(h, 1.0f, 1.0f));\n" \
	"		color.rgb = SRGBToLinear(color.rgb);\n" \
	"	}\n" \
	"	else if (rect.flags & UiRectFlags_sat_val_picker)\n" \
	"	{\n" \
	"		float s = uv.x;\n" \
	"		float v = uv.y;\n" \
	...
	"\n" \
	"		if (rect.flags & UiRectFlags_tonemap)\n" \
	"		{\n" \
	"			tex_col.rgb = 1.0 - exp(-tex_col.rgb);\n" \
	"		}\n" \
	"\n" \
	"		if (rect.flags & UiRectFlags_blend_text)\n" \
	"		{\n" \
	...
	"}\n" \
	"\n" \
	"float4 ui_heatmap_ps(VSOut IN) : SV_Target\n" \
	"{\n" \
	"	UIRect rect = draw.rects.Get()[IN.id];\n" \
	"\n" \
	"	Rect2 clip_rect = Decode(rect.clip_rect);\n" \
