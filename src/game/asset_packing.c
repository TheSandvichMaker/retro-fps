...
fn_local uint32_t get_pointer_offset_u32(void *base, void *data)
{
	if (data == NULL)
	{
		return 0;
...
fn_local void dispatch_pack_jobs(pack_context_t *context)
{
	...
	header->textures_offset = get_pointer_offset_u32(header, textures);
	header->sounds_offset   = get_pointer_offset_u32(header, sounds);

	for (size_t i = 0; i < sb_count(context->jobs); i++)
	{
		add_job_to_queue(low_priority_job_queue, process_pack_job, &context->jobs[i]);
	}
...
void pack_assets(string_t source_directory)
{
	...
	string_t pack_file_data = {
		.data  = pack_file->arena.buffer,
		.count = m_size_used(&pack_file->arena),
	};
...
void process_pack_job(job_context_t *job_context, void *userdata)
{
	...
	pack_job_t     *job     = userdata;
	pack_context_t *context = job->context;
	...
	switch (job->kind)
	{
		...
		{
			pack_texture_t *textures = (pack_texture_t *)(pack_arena->buffer + header->textures_offset);
			...
			texture->data_offset  = get_pointer_offset_u32(header, image_data);
		} break;

		case PackAsset_sound:
		{
			pack_sound_t *sounds = (pack_sound_t *)(pack_arena->buffer + header->sounds_offset);
			...
			sound->data_offset   = get_pointer_offset_u32(header, sound_data);
		} break;
	}
